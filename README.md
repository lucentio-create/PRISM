...
DOCUMENT
   │
   ▼
APC Analysis Agent  +  AWP Analysis Agent   (parallel)
   │                        │
   └──────────┬─────────────┘
              ▼
      GUARDRAIL Agent 🛡️  ──(one resolution loop max)──┐
              │                                         │
              ▼                                         │
      QC Agent / Second Review Agent                    │
              │                                         │
              ▼                                         │
      HUMAN REVIEW 👤  ◄── unresolved divergence routes here directly
              │
              ▼
      PRIVILEGE LOG Entry Agent 📋 ──► AUDIT TRAIL 📊
```
...
1. **Checks the reasoning** of APC and AWP — not just their conclusions, but whether each agent correctly applied its own test.
2. **Resolves divergence** — when APC and AWP disagree, Guardrail analyzes the conflict, makes a decision, and returns the case to both agents *once* with specific instructions for resolving it. A single loop, capped to prevent endless back-and-forth. If the two agents still cannot converge after that one loop, Guardrail escalates directly to Human Review.
